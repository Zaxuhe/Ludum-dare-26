...
inline std::string encrypt_text(std::string key, char* _text, int _size)
{
...
inline std::string decrypt_text(std::string key, char* _text, int _size)
{
	CBlowFish fish((unsigned char*)key.c_str(),key.length());
	int cnt = 0;
	int i = 0;
...
inline std::string decrypt_file(std::string key, std::string file_name)
{
	//we open the file
	...
	if (file.is_open())
	{
		...
		//we decrypt the file :)
		file_out = decrypt_text(key,memblock,size);
