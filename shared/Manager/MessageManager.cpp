...
void Message::SetDeliveryTime( int deliveryTimeMS )
{
	...
	{
		m_deliveryTime = deliveryTimeMS + GetBaseApp()->GetTick();
	}
}
...
void MessageManager::Update()
{
	while (!m_systemMessages.empty() &&  (*m_systemMessages.begin())->GetDeliveryTime() <= GetBaseApp()->GetTick())
	{
		...
		/*if ((game_paused_ == true && m->m_stop_on_pause_ == true) == false)
		{*/
			Deliver(m);
			delete m;
		/*}
		...
		{
			m->SetDeliveryTime((GetBaseApp()->GetTick()-m->GetDeliveryTime()+2));
			Send(m);
		}*/
	}
	...
	while (!m_gameMessages.empty() && (*m_gameMessages.begin())->GetDeliveryTime() <= GetBaseApp()->GetGameTick())
	{
		...
		{
			//LogMsg("MEssage Paused %d %d %d %d",m->GetDeliveryTime(), GetBaseApp()->GetTick(),GetBaseApp()->GetGameTick()-m->GetDeliveryTime(),m->GetDeliveryTime()+GetBaseApp()->GetGameTick()-m->GetDeliveryTime());
			m->SetDeliveryTime((GetBaseApp()->GetGameTick()-m->GetDeliveryTime()+2));
			m->checked_ = true;
			Send(m);
		}*/
	}

	//we now add the time passed if the game was paused, so we never enter into it
	if (game_paused_ == true && m_gameMessages.empty() == false)
	{
		
		std::list <Message*>::iterator it;
		std::list <Message*>::iterator it2;
		for (it=m_gameMessages.begin(); it!=m_gameMessages.end(); ++it)
		{
			Message *m = (*it);
			if (m->checked_)
			{
			...
			if (m->m_stop_on_pause_ == true)
			{
				if (m->paused_diff_ == -1) //we have not paused it yet
				{
					m->paused_diff_ = m->GetDeliveryTime()-GetBaseApp()->GetGameTick();
				}
				it2 = --it;
				++it;
				m_gameMessages.erase(it);
				m->SetDeliveryTime(m->paused_diff_);
				m->checked_ = true;
				Send(m);
				it=it2;
			}
		...
		for (it=m_systemMessages.begin(); it!=m_systemMessages.end(); ++it)
		{
			Message *m = (*it);
			if (m->checked_)
			{
...
void MessageManager::CallEntityFunctionZax( Entity *pEnt, int timeMS, const string &funcName, const VariantList *v, bool pauses_game, eTimingSystem timing )
{
...
void MessageManager::pause_game(bool _state)
{
	game_paused_ = _state;
