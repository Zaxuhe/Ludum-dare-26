...
EntityComponent * MorphToVec2EntityZax(Entity *pEnt, string targetVar, CL_Vec2f vTargetSize, unsigned int speedMS, eInterpolateType interpolateType,  int delayBeforeActionMS)
{
	EntityComponent* pComp = SetupInterpolateComponent(pEnt, "ic_" + targetVar, targetVar, vTargetSize, speedMS, delayBeforeActionMS, interpolateType);
	InterpolateComponent* iComp = (InterpolateComponent*)pComp;
	iComp->is_pausable_ = true;
	return pComp;
}
...
EntityComponent * MorphToColorEntity_Zax(Entity *pEnt, std::string component_name_, unsigned int color, int timeMS, int delayBeforeActionMS)
{
	return SetupInterpolateComponent(pEnt, "ic_" + component_name_, component_name_, uint32(color), 2000, 0, INTERPOLATE_SMOOTHSTEP_AS_COLOR);
}
