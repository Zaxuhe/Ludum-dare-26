...
void RenderBatcher::clean()
{
	m_verts.clear();
	m_batchEvents.clear();
}

void RenderBatcher::Flush_no_clean(eFlushMode mode)
{
	if (mode == FLUSH_SETUP || mode == FLUSH_SETUP_RENDER_UNSETUP || mode == FLUSH_SETUP_RENDER)
	{
	...
	if (mode == FLUSH_RENDER || mode == FLUSH_SETUP_RENDER_UNSETUP || mode == FLUSH_SETUP_RENDER)
	{
		...
		int curPrim = 0;
		RenderBatchEvent *event;
		std::deque<RenderBatchEvent>::iterator it = m_batchEvents.begin();

		while (it != m_batchEvents.end())
		{
			event = &(*it);
