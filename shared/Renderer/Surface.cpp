...
//the diference is that it repeats the texture instead of streaching
void Surface::BlitEx2_zax(rtRectf dst, rtRectf src, CL_Vec2f displacement, unsigned int rgba, float rotation, CL_Vec2f vRotatePt)
{
        if (!IsLoaded()) return;
		...
		glViewport(0,0,GetOriginalScreenSizeX(),GetOriginalScreenSizeY());
...
if (GetBaseApp()->GetDisableSubPixelBlits())
{
		...
		//number of times this fits
		float right_times =(float)GetOriginalScreenSizeX()/(m_texWidth);
		float top_times = (float)GetOriginalScreenSizeY()/(m_texHeight);

		float left_mod = 0;
		float btm_mod = 0;
		if (displacement.x != 0)
		{
			left_mod = displacement.x/(m_texWidth);
			left_mod = fmodf(left_mod, 1.0f);
		}

		if (displacement.y != 0)
		{
			btm_mod = (float)displacement.y/(m_texHeight);
			btm_mod = fmodf(btm_mod, 1.0f);
		}
		//LogMsg("%f %f", right_times,top_times);
        //vert 0
		pTex->x = left_mod;
        pTex->y = btm_mod;
        pTex++;
        ...
        //vert 1
        pTex->x = right_times+left_mod;
        pTex->y = btm_mod;
        pTex++;

        //vert 2
        pTex->x = right_times+left_mod;
        pTex->y = top_times*-1+btm_mod;
        pTex++;
