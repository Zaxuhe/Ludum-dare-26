...
* creado por Zaxuhe
*
* Dibujamos una aniamcion
*
* @param x Posicion x
* @param y Posicion y
* @param frameX donde inicia el blit dentro de la imagen
* @param frameY donde inicia el blit dentro de la imagen
* @param frameSizeX el tamaño del frame
* @param frameSizeY el tamaño del frame
* @param vScale Escala
* @param alignment Alienacion
* @param rgba color
* @param rotation rotacion
* @param vRotationPt el punto en el que rota
* @param flipX voletamos en x
* @param flipY volteamos en y
*/
void SurfaceAnim::BlitScaledAnimZax( float x, float y, float frameX , float frameY, int frameSizeX , int frameSizeY, CL_Vec2f vScale, eAlignment alignment /*= ALIGNMENT_CENTER*/,
								 unsigned int rgba /*= MAKE_RGBA(255,255,255,255)*/, float rotation /* = 0*/, CL_Vec2f vRotationPt/*CL_Vec2f(0,0))*/, bool flipX /*= false*/, bool flipY /*= false*/)
{
	if (vScale.x == 0 && vScale.y == 0) return;
	/*if (GetFrameWidth() == GetWidth() && GetFrameHeight() == GetHeight() && !flipX && !flipY) 
	{
	...
	src.left = frameX;
	src.top = frameY;
	src.right = src.left+frameSizeX;
	src.bottom = src.top + frameSizeY;
	
	rtRectf dst(0,0, frameSizeX, frameSizeY);
	...
	if (alignment != ALIGNMENT_UPPER_LEFT)
	{
		vStart -= GetAlignmentOffset(CL_Vec2f(frameSizeX, frameSizeY), alignment);
	}
...
/**
Definicion H
		void BlitScaledAnimZax( float x, float y,  float frameX, float frameY, int frameSizeX , int frameSizeY, CL_Vec2f vScale, eAlignment alignment = ALIGNMENT_CENTER, unsigned int rgba  = MAKE_RGBA(255,255,255,255), float rotation=0, CL_Vec2f vRotationPtScreenCoords = CL_Vec2f(0,0),
